...
console.log(
  `task 4: vowel count is ${counter[0]}, Consonants count is ${counter[1]}`
);
console.log(``);
...
function getLettersFrequencyObj(str) {
  let frequencies = {};
  for (let i = 0; i < str.length; i++) {
    if (frequencies[str[i].toUpperCase()]) {
      frequencies[str[i].toUpperCase()]++;
    } else {
      frequencies[str[i].toUpperCase()] = 1;
    }
  }
...
function countFreq(str) {
  let freqObject = {};
  for (let i = 0; i < str.length; i++) {
    if (!freqObject[str[i].toUpperCase()]) {
      freqObject[str[i].toUpperCase()] = 1;
      for (let j = i + 1; j < str.length; j++) {
        if (str[i].toUpperCase() === str[j].toUpperCase()) {
          freqObject[str[i].toUpperCase()]++;
        }
      }
    }
  }
...
console.log(
  `task 11: the string "radar" is palindrome is ${isPalindrome(`raadar`)}`
);
console.log(
  `task 11: the string "radar" is palindrome is ${isPalindromeNested(`raadar`)}`
);
console.log(``);

//task 12
let arr1 = [1, 3, 5, 7, 9, 11],
  arr2 = [1, 2, 3, 4, 5, 10, 11, 12, 13, 14]; //[1, 3, 5, 11] is the common
let commonArr = [];
